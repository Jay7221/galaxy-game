...
class MainWidget(RelativeLayout):
    ...
    best_score = 0
    score_txt = StringProperty(f"SCORE: {current_y_loop}")
    best_score_txt = StringProperty(f"BEST SCORE: {best_score}")
    ...
    def init_audio(self):
        ...
        self.sound_gameover.volume = .25
        self.sound_gameover_impact.volume = .6

    def reset_game(self):
        self.best_score = max(self.best_score, self.current_y_loop)
        self.current_offset_y = 0
        self.current_y_loop = 0
        self.current_offset_x = 0
        self.current_speed_x = 0
        self.score_txt = f"SCORE: {self.current_y_loop}"
        self.best_score_txt = f"BEST SCORE: {self.best_score}"
    ...
    def update_ship(self):
        ...
        ship_half_width = self.SHIP_WIDTH * self.height
        ship_height = self.SHIP_HEIGHT * self.height

        x1, y1 = (center_x - ship_half_width, base_y)
        x2, y2 = (center_x, base_y + ship_height)
        x3, y3 = (center_x + ship_half_width, base_y)
    ...
    def generate_tile_coordinates(self):
        ...
        if len(self.tiles_coordinates) > 0:
            last_y = self.tiles_coordinates[-1][1]
    ...
    def update(self, dt):
        ...
        if not self.state_game_over and self.state_game_started:
            ...
            self.current_offset_y += speed_y * time_factor * self.difficulty
            ...
            self.difficulty = 1 + self.current_y_loop / 400
